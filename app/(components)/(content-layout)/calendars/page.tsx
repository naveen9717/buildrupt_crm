...
const Calendars: React.FC<CalendarsProps> = () => {
  ...
  const [loading, setLoading] = useState(true); // ⬅️ drives skeletons
  ...
  const handleUpdate = async (formSnapshot: any) => {
  ...
  const rightContainerRef = useRef<any>(null);
  const topContainerRef = useRef<any>(null);
  const bottomContainerRef = useRef<any>(null);
  const lastContainerRef = useRef<any>(null);

  const leftButtonRef = useRef<any>(null);
  const rightButtonRef = useRef<any>(null);
  const topButtonRef = useRef<any>(null);
  const bottomButtonRef = useRef<any>(null);
  ...
  // (original demo events kept here if you still use them somewhere)
  const events = [
    {
      id: 1,
      date: "2025-11-04",
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true); // ⬅️ show skeletons while fetching
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/calendars/`);
      if (!res.ok) {
  ...
  function useEventsByMonth(projectsArg: any[]) {
    return useMemo(() => {
      const grouped: Record<string, any[]> = {};
      projectsArg.forEach((project) => {
  ...
  // ----------------- Skeleton components (ADDED ONLY) -----------------
  const HeaderSkeleton = () => (
    <Card className="custom-card">
      <Card.Body className="p-3">
        <div className="d-sm-flex align-items-center flex-wrap gap-3 justify-content-between">
          <div className="d-flex gap-3 w-sm-50 mb-sm-0 mb-3">
  ...
  const MonthSkeleton = () => (
    <>
      <h6 className="text-xl fw-bold mb-2">
        <Skeleton width={180} height={18} />
      </h6>
      <div className="VYZOR-kanban-board">
        {Array.from({ length: 14 }).map((_, i) => (
          <div key={i} className="kanban-tasks-type new">
                    ...
                    <div className="VYZOR-kanban-board">
                      {days.map((day, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className={`kanban-tasks-type ${getCardColor(
                              WeekDay
                            )}`}
                          >
                            <div className="pe-3 mb-3">
