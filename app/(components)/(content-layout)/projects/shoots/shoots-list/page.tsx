...
const ShootsTables: React.FC<DataTablesProps> = () => {
  ...
  const fetchItems = async () => {
    fetch(`${process.env.NEXT_PUBLIC_URL}/api/projects/shoots`)
      .then((res) => res.json())
  ...
  const columns = [
    ...
    {
      name: "DURATION",
      selector: (row) => capitalizeFirst(row.duration),
      sortable: true,
    },
  ...
  const handleEdit = (project_id: string, shootid: string) => {
    router.push(
      `${process.env.NEXT_PUBLIC_URL}/projects/${project_id}/shoots/${shootid}`
    );
    // You can trigger a modal or setForm(item)
  };

  const handleDelete = async (id: string) => {
    await fetch(`${process.env.NEXT_PUBLIC_URL}/api/projects/shoots`, {
      method: "DELETE",
