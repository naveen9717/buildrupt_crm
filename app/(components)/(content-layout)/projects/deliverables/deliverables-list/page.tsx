...
const DeliverablesTables: React.FC<DataTablesProps> = () => {
  ...
  const fetchItems = async () => {
    fetch(`${process.env.NEXT_PUBLIC_URL}/api/projects/deliverables`)
      .then((res) => res.json())
  ...
  const columns = [
    ...
    {
      name: "COST",
      selector: (row) => capitalizeFirst(row.cost),
      sortable: true,
    },
    {
      name: "STATUS",
  ...
  const handleEdit = (project_id: string, deliverableid: string) => {
    router.push(
      `${process.env.NEXT_PUBLIC_URL}/projects/${project_id}/deliverables/${deliverableid}`
    );
  ...
  const handleDelete = async (id) => {
    await fetch(`${process.env.NEXT_PUBLIC_URL}/api/projects/deliverables`, {
      method: "DELETE",
  ...
  return (
    <Fragment>
      <Seo title="Shoots Data" />

      <Pageheader
        title="Tables"
        currentpage="Shoots Data"
        activepage="Shoots Data"
      />
...
export default DeliverablesTables;
