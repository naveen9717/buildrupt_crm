  ...
  try {
    ...
    for (const photo of form2) {
      await db.query(
        ...
        [project_id, ShootId, photo.photo_crew, photo.photo_role]
      );
    }
    ...
    for (const video of form3) {
      await db.query(
        ...
        [project_id, ShootId, video.video_crew, video.video_role]
      );
    }
...
export async function PUT(req) {
  try {
    ...
    await db.execute("UPDATE shoots SET name = ?, price = ? WHERE id = ?", [
      name,
      price,
      id,
    ]);
